...
export const getConversationInfo = async (conv_id, conv_type) => {
  const { code, json_param } = await timRenderInstance.TIMConvGetConvInfo({
...
export const getLoginUserID = async () => {
  const { code, json_param } = await timRenderInstance.TIMGetLoginUserID();
...
>): Promise<MsgResponse> => {
  const res = await timRenderInstance.TIMMsgSendMessageV2({
    ...
    conv_type: convType,
    params: {
      message_elem_array: messageElementArray as any,
...
>): Promise<MsgResponse> => {
  ...
  return {data:{code:res.code,desc:"",json_params:res.json_params,user_data:""}};
}
...
export const searchTextMessage = async (params: {
  ...
  // const { json_params } = await timRenderInstance.TIMMsgSearchLocalMessages({
    const res = await timRenderInstance.TIMMsgSearchLocalMessages({
    params: {
...
export const searchFriends = async (params: {
  ...
  const { json_params } = await timRenderInstance.TIMFriendshipSearchFriends({
    params: {
  ...
  console.log("searchFriends", json_params || []);
  return json_params || [];
};
...
export const modifyGroupInfo = async (params: {
  ...
  if (!results.find((item) => item?.code !== 0)) {
    return {};
  }
...
export const getBlackList = async () => {
  const { code, desc, json_params }  = await timRenderInstance.TIMFriendshipGetBlackList({
    user_data: ''
  });
...
export const addFriendsToBlackList = async (friendsArray: [string]) => {
  const  { code, desc, json_params }  = await timRenderInstance.TIMFriendshipAddToBlackList({
    params: friendsArray,
    user_data: ''
  });
  if (code === 0) {
...
export const removeFriendsFromBlackList = async (friendsArray: [string]) => {
  const  { code, desc, json_params }  = await timRenderInstance.TIMFriendshipDeleteFromBlackList({
    params: friendsArray,
    user_data: ''
  });
  if (code === 0) {
...
export const getFriendList = async (): Promise<FriendProfile[]> => {
  const  data  = await timRenderInstance.TIMFriendshipGetFriendProfileList({
    user_data: "123",
  });
  const { code, desc, json_params } = data;
  console.log("TIMFriendshipGetFriendProfileList", json_params);
  if (code === 0) {
    return json_params;
  }
  return [];
...
export const modifyGroupOwner = async ({
  ...
  const  { code, desc } = await timRenderInstance.TIMGroupModifyGroupInfo({
    params: {
      group_modify_info_param_group_id: groupId,
...
export const findMsg = async ({
  ...
  console.log(res,"findmsg");
  const { code, desc, json_params } = res;
  if (code === 0) {
    const result = json_params;
    return result;
  }
  throw new Error(desc);
}
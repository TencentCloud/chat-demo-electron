...
export const MessageView = (props: Props): JSX.Element => {
    ...
    const handleContextMenuEvent = async (e, message: State.message, element) => {
        e.preventDefault();
        const serverTime = await timRenderInstance.TIMGetServerTime();
    ...
    const handleMessageReSend =async (params) => {
        ...
        const {code,json_params} = await timRenderInstance.TIMMsgSendMessage({
            conv_id,
            conv_type,
            params
        })
        console.log(code,json_params,"messageview")
        if(code===0){
            console.log("messageview code = 0")
    ...
    const getMoreMsg = async () => {
        if(!noMore){
            ...
            const messageResponse = await getMsgList(message_conv_id, message_conv_type,msg);
            if(messageResponse.length>0){
                setAnchor(message_msg_id) 
            }else{
                setNoMore(true)
            }
            const addTimeDividerResponse = addTimeDivider(messageResponse.reverse() as unknown as State.message[]);
            const payload = {
                convId: message_conv_id,
